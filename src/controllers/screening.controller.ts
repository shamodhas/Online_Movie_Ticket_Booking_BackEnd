...
export const createScreening = async (req: Request, res: Response) => {
    ...
    try {
        const newScreening = new Screening({ movieId, theaterId, startTime, endTime });
...
export const updateScreening = async (req: Request, res: Response) => {
    try {
        const updatedScreening = await Screening.findByIdAndUpdate(req.params.screeningId, req.body, { new: true });
        if (!updatedScreening) {
            return res.status(404).json({ message: 'Screening not found' });
        }
...
export const deleteScreening = async (req: Request, res: Response) => {
    try {
        const deletedScreening = await Screening.findByIdAndDelete(req.params.screeningId);
        if (!deletedScreening) {
            return res.status(404).json({ message: 'Screening not found' });
        }
